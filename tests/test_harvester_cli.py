...
def test_dotenv_token_pickup(temp_config_file, monkeypatch):
    ...
    # Mock the Client and harvester components
...
def test_dotenv_token_explicit_override(temp_config_file, monkeypatch):
...
def test_missing_token_raises_error(temp_config_file, monkeypatch):
...
def test_cli_with_only_required_parameters(temp_config_file, monkeypatch):
    ...
    with patch('molgenis_fdp_harvester.harvester.Client') as mock_client_class, \
        ...
        mock_create_harvester.assert_called_once()
        call_args = mock_create_harvester.call_args
        ...
        mock_execute_harvest.assert_called_once()
        harvest_call_args = mock_execute_harvest.call_args
...
def test_create_harvester_valid_types(input_type, expected_class):
    ...
    harvester_config = {'auto_create_datasetseries': True}
...
def test_create_harvester_invalid_type():
