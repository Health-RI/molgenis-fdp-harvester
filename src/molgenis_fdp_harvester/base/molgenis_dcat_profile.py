...
class MolgenisEUCAIMDCATAPProfile(RDFProfile):
    ...
    def handle_pids(self, dataset_dict: Dict):
        ...
        if pid_bool:
            ...
            # If 'identifier' is a hyperlink, it is a proper PID, and will therefore be cleaned before submitted to 'id'
            dataset_dict['id'] = str(dataset_dict['identifier']).replace(':', '-').replace('/','-')
        else:
            ...
            dataset_dict['identifier'] = f"{self.config['pid_service_url']}/{dataset_dict['id']}"
